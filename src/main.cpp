...
#if defined(NDEBUG)
# error "Mitocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("82d3bd2ce48363c9bbd56238c77b05814274322095551ef9c910d93d27662153");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mitocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mitocoin Signed Message:\n";
...
{
    // Mitocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Mitocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Mitocoin: 3.5 days
static const int64 nTargetSpacing = 0.5 * 60; // Mitocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mitocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mitocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("82d3bd2ce48363c9bbd56238c77b05814274322095551ef9c910d93d27662153");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1513784917;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085427232;
        ...
        assert(block.hashMerkleRoot == uint256("d51e1ad3f34b0cbcb34d5bfe953d9f59c7603dda3e0b32dd5d4163b33c74ffb2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc9, 0xb8, 0xd4 }; // Mitocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MitocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MitocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MitocoinMiner(CWallet *pwallet)
{
    printf("MitocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mitocoin-miner");
    ...
    try { loop {
        ...
        printf("Running MitocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MitocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MitocoinMiner, pwallet));
}
